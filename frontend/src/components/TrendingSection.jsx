...
const TrendingSection = ({ onBuy, onView }) => {
    ...
    const nextSlide = () => {
        setCurrentIndex((prev) => (prev + 1) % Math.max(1, trendingNotes.length - 2));
    };

    const prevSlide = () => {
        setCurrentIndex((prev) => (prev - 1 + Math.max(1, trendingNotes.length - 2)) % Math.max(1, trendingNotes.length - 2));
    };
            ...
            <div className="mb-12">
                <div className="animate-pulse">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="h-64 bg-gray-200 dark:bg-[#2a2a2a] rounded-xl"></div>
                        ))}
                    </div>
                </div>
            </div>
            ...
            <div className="relative overflow-hidden">
                ...
                >
                    {trendingNotes.map((note) => (
                        <div key={note._id} className="min-w-[calc(33.333%-1rem)] flex-shrink-0">
